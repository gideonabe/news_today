...
const Content = () => {
  ...
  // Keep Redux state in sync with URL
  useEffect(() => {
    const categoryFromQuery = searchParams?.get("category") || "All";
  ...
  return (
    <section className="max-w-[90%] md:max-w-[75%] mx-auto mt-8 mb-8 md:mb-18">
      ...
      {!loading && error && (
        <div className="text-center py-10">
          <p className="text-red-500 mb-4 font-semibold text-lg">Oops! Something went wrong while fetching the articles.</p>
          <p className="text-gray-600 mb-6">{error}</p>
          <button
            onClick={() => {
              setError(null); // clear previous error
              if (debouncedQuery) {
                // Trigger fetch by updating debouncedQuery
                setDebouncedQuery(debouncedQuery); 
              } else {
                // Trigger fetch by updating selectedCategory
